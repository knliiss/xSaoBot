...
public class MessagePackService {
    ...
    public MessagePack buyMessagePack(String packId, long userId) {
        ...
        if (user.getBalance() < pack.getCost()) throw new EntityException("Insufficient balance to buy the message pack");
        balanceService.withdraw(userId, pack.getCost());
        user.getOwnedMessagePacksIds().add(pack.getId());
        userService.save(user);
        return pack;
    }
    
    public double getMessagePackPrice(String messagePackId) {
        loadIfNeeded();
        var pack = getById(messagePackId);
        if (pack == null) throw new EntityException.EntityNotFoundException("MessagePack not found");
        return pack.getCost();
    }
    
    public boolean isPackOwned(long userId, String packId) {
        var user = userService.findById(userId);
        if (user == null) throw new EntityException.EntityNotFoundException("User not found");
        return user.getOwnedMessagePacksIds().stream()
                .anyMatch(id -> id.equalsIgnoreCase(packId));
    }
}